...
export default class BackTop extends BackTopUtil {
	constructor(option = {}) {
		...
		this.debug = option.debug === true ? true : false
		this.option = option
	...
	toTop(speed) {
		...
		const timer = setInterval(() => {
			...
			if (curHeight <= 0.1) {
				this.debug ? console.log("It's at the top...") : null
				clearInterval(timer)
				this.backTopBtn.style.display = "none"
			}
		}, 10)
	...
	iteraQueryTargetDom(target) {
		return new Promise((resolve, reject) => {
			...
			let queryInterval = setInterval(() => {
				...
				this.debug ? console.log("【BackTop】: Getting the target element.") : null
				...
				if (
					...
					this.debug ? console.log("【BackTop】: Intercepted the target element.") : null
